...
class Agent:
    ...
    def act(self, environment, points, dead):
        ...
        if self._train:
            for action in self.actions:
                N_value = self.N[s_prime + (action,)]
                Q_value = self.Q[s_prime + (action,)]
                if N_value < self.Ne:
                    f_function.append(1)
                else:
            ...
            _act = max(best_actions, key=lambda x: (f_function[x], -x))
        else:
            Q_value = self.Q[s_prime]
            _act = np.argmax(Q_value)
        self.s = s_prime
        self.a = _act
        return _act
