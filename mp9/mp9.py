...
def main(args):
    best_lr, best_h, best_m, best_acc = 0, 0, 0, 0
    for lr in np.arange(0.01, 0.1, 0.01):
        for h in np.arange(20, 200, 10):
            m = 0
            # for m in np.arange(0.1, 1, 0.1):
            print("learning rate:", lr, ", hidden size:", h, ", momentum:", m)
            ...
            L, predicted_labels, net = p.fit(train_set, train_labels, dev_set, args.epochs, hidden_size=h, learning_rate=lr, momentum=m)
            ...
            if accuracy > best_acc:
                best_lr, best_h, best_m, best_acc = lr, h, m, accuracy
            print("\n Confusion Matrix = \n {}".format(conf_m))
            print('\n Parameters = {} \n'.format(num_parameters))
    print("best learning rate:", best_lr, ", best hidden size:", best_h, ", best momentum:", best_m, ", accuracy:", best_acc)
