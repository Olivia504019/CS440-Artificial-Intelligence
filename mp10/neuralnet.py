...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, hidden_size_1, hidden_size_2):
        """
        ...
        self.model = nn.Sequential(
            nn.Conv2d(3, hidden_size_1, 5),
            nn.ReLU(),
            nn.MaxPool2d(2),
            nn.Conv2d(hidden_size_1, hidden_size_2, 5),
            nn.ReLU(),
            nn.MaxPool2d(2)
        )
        self.fc = nn.Linear(hidden_size_2 * 4 * 4, 4)
...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100, hidden_size_1 = 60, hidden_size_2 = 60, learning_rate = 0.0007):
    """ 
    ...
    model = NeuralNet(learning_rate, nn.CrossEntropyLoss(), hidden_size_1, hidden_size_2)
    losses = []
    ...
    yhats = np.argmax(model(dev_set).detach().cpu().numpy(), axis = 1)
