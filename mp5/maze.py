...
class Maze:
    ...
    def is_objective(self, waypoint):
        ...
        """
        return waypoint in self.get_objectives()
    ...
    # Returns the list of objective positions of the maze, formatted as (x, y) tuples
    ...
    def filter_valid_waypoints(self):
        ...
        for shape in range(len(self.alien.get_shapes())):
            ...
            for waypoint in self.get_waypoints():
                alien = self.create_new_alien(*waypoint, shape)
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        """Find the k nearest valid neighbors to the cur_waypoint from a list of 2D points.
            Args:
                cur_waypoint: (x, y) current position
        ...
        for waypoint in self.get_valid_waypoints()[cur_shape]:
            if waypoint == cur_waypoint:
                continue
            if self.is_valid_move((*cur_waypoint, cur_shape), (*waypoint, cur_shape)):
                neighbors_with_distance.append((euclidean_distance(cur_waypoint, waypoint),  waypoint))
        nearest_neighbors = []
        for _, neighbor in sorted(neighbors_with_distance)[:self.k]:
    ...
    def is_valid_move(self, start, end):
        ...
        """
        if end[2] < 0 or end[2] >= len(self.alien.get_shapes()):
            return False
        alien = self.create_new_alien(*end)
        if does_alien_touch_wall(alien, self.walls):
            return False
        return not does_alien_path_touch_wall(self.create_new_alien(*start), self.walls, end)
