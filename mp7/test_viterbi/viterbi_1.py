...
def training(sentences):
    ...
    tag_trans_cnts = defaultdict(lambda: defaultdict(int))
    emit_cnts = defaultdict(lambda: defaultdict(int))
    for sentence in sentences:
        prev_tag = sentence[0][1]
        for word, curr_tag in sentence[1:]:
    ...
    for tag in emit_cnts.keys():
        for word in emit_cnts[tag].keys():
            emit_prob[tag][word] = math.log(emit_cnts[tag][word]) - math.log(sum(emit_cnts[tag].values()))
...
def viterbi_stepforward(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob):
    ...
    """
    # print(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob, sep="\n---------------------\n", end='\n---------------------\---------------------\---------------------\n')
...
def viterbi_1(train, test, get_probs=training):
    ...
    for sen in range(len(test)):
        ...
        for i in range(length - 1, -1, -1):
            predict.append((sentence[i], last_tag))
            if i > 0:
                last_tag = total_predict_tag_seq[i][last_tag]
